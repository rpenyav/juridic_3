...
import { PaginatedResponse } from 'src/app/interfaces/paymethods';
...
export class AutocompleteComponent implements OnInit {
  ...
  getValue(o:any, key:string):any{
    ...
    if(key.toString().indexOf(".")>-1) {
      key.toString().split(".").forEach((k:string)=>{
        tmpK += "['" + k + "']";
      })
      return eval(tmpK);
    };
  ...
  selectEntry(entry:any) {
   ...
   if(this.structure?.autocomplete?.returnField == null){
    this.onEntrySelected.emit(entry);
      ...
      let r = (typeof(this.structure?.autocomplete?.returnField) == "string")?this.structure?.autocomplete?.returnField : "id";
