...
import { ReturnReasons } from '../../../interfaces/return_reasons';
...
@Component({
  selector: 'app-postal-codes-detail',
  templateUrl: './postal-codes-detail.component.html',
  styleUrls: ['./postal-codes-detail.component.scss'],
})
export class PostalCodesDetailComponent implements OnInit {
  endpoints = getApiEndpoints();
  titol: string = '';

  id: string | null = null;
  registerDetail: ReturnReasons | null = null;
  originalData: ReturnReasons | null = null;
  ...
  redirectRoute: string = 'return-reasons'; //ruta retorn al llistat
  ...
  toggleSave(): void {
    ...
    if (updatedDataString) {
      const updatedData: ReturnReasons = JSON.parse(updatedDataString);
  ...
  toggleInsert(): void {
    ...
    if (insertDataString) {
      const insertData: ReturnReasons = JSON.parse(insertDataString);
  ...
  loadRegisterDetail(): void {
    ...
    if (!isNaN(idNum)) {
      ...
      this.generalService
        .getRegisterTypeById<ReturnReasons>(dynamicEndpoint, idNum)
        .subscribe({
          next: (register: ReturnReasons | null) => {
  ...
  submitNewRecord(insertData: any, language: number): void {
    ...
    this.generalService
      .createRegisterType<ReturnReasons>(
        customEndpoint,
        dataToSend as ReturnReasons
      )
      .subscribe({
        next: () => {
          Swal.fire({
   ...
   * @param updatedData
   */
  saveChanges(updatedData: ReturnReasons): void {
    if (this.registerDetail && this.id) {
      ...
      this.generalService
        .updateRegisterType<ReturnReasons>(
          customEndpoint,
          Number(this.id),
          updatedData
        )
