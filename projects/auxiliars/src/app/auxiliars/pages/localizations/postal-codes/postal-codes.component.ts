...
import {
  PostalCodes,
  PaginatedResponse,
} from '../../../interfaces/localizations/postal-codes';
...
export class PostalCodesComponent implements OnInit {
  ...
  detailUrl: string = 'postal-codes';
  sortField: string = 'code'; // Camp per defecte per la ordenacio
  icono: string = 'postal-codes';
  ENDPOINT = `${this.endpoints.POSTAL_CODES}`;
  addressTypesData: PostalCodes[] = [];
  filteredRegistersData: PostalCodes[] = [];
  titolKey = 'SUBMENU.POSTAL_CODES';
  ...
  updateTableData(data: PostalCodes[]): void {
  ...
  create(addressTypesData: PostalCodes) {
    console.log('Enviando datos al servidor:', addressTypesData);
    this.generalService
      .createRegisterType<PostalCodes>(this.ENDPOINT, addressTypesData)
      .subscribe({
        next: (response) => {
   ...
   * @param addressTypes
   */
  delete(addressTypes: PostalCodes) {
    ...
    showCustomAlert(this.i18nService, {
      ...
      if (result.isConfirmed) {
        this.generalService
          .deleteRegisterType<PostalCodes>(this.ENDPOINT, addressTypes.id!)
          .subscribe({
            next: (response) => {
