...
import { Comarcs } from '../../interfaces/comarcs';
import { GeneralService } from '../../services/general.service';
import { environment } from 'projects/auxiliars/src/environments/environment';

@Component({
  selector: 'app-comarcs-detail',
  templateUrl: './comarcs-detail.component.html',
  styleUrls: ['./comarcs-detail.component.scss'],
})
export class ComarcsDetailComponent implements OnInit {
  assetsBaseUrl = '/assets/';
  endpoints = getApiEndpoints();
  ENDPOINT = `${this.endpoints.COMARCS_ENDPOINT}`;
  iconoS: string = 'localitTypes';
  icono: string = 'comarca';
  redirectRoute: string = 'regions'; //ruta retorn al llistat
  id: string | null = null;
  registerDetail: Comarcs | null = null;
  originalData: Comarcs | null = null;
  ...
  toggleSave(): void {
    ...
    if (updatedDataString) {
      const updatedData: Comarcs = JSON.parse(updatedDataString);
  ...
  toggleInsert(): void {
    ...
    if (insertDataString) {
      const insertData: Comarcs = JSON.parse(insertDataString);
  ...
  loadRegisterDetail(): void {
    ...
    if (!isNaN(idNum)) {
      ...
      this.generalService
        .getRegisterTypeById<Comarcs>(dynamicEndpoint, idNum)
        .subscribe({
          next: (register: Comarcs | null) => {
   ...
   * @param insertData
   */
  submitNewRecord(insertData: Comarcs): void {
    ...
    this.generalService
      .createRegisterType<Comarcs>(customEndpoint, insertData)
      .subscribe({
        next: () => {
          Swal.fire({
            icon: 'success',
   ...
   * @param updatedData
   */
  saveChanges(updatedData: Comarcs): void {
