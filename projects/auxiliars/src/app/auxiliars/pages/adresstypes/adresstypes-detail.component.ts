...
@Component({
  ...
  templateUrl: './adresstypes-detail.component.html',
...
export class AdresstypesDetailComponent implements OnInit {
  ...
  id: string | null = null;
  registerDetail: AddressType | null = null;
  originalData: AddressType | null = null;
  ...
  toggleSave(): void {
    ...
    if (updatedDataString) {
      const updatedData: AddressType = JSON.parse(updatedDataString);
  ...
  toggleInsert(): void {
    ...
    if (insertDataString) {
      const insertData: AddressType = JSON.parse(insertDataString);
  ...
  loadRegisterDetail(): void {
    ...
    if (!isNaN(idNum)) {
      ...
      this.generalService
        .getRegisterTypeById<AddressType>(dynamicEndpoint, idNum)
        .subscribe({
          next: (register: AddressType | null) => {
   ...
   * @param insertData
   */
  submitNewRecord(insertData: AddressType): void {
    ...
    this.generalService
      .createRegisterType<AddressType>(customEndpoint, insertData)
      .subscribe({
        next: () => {
          Swal.fire({
            icon: 'success',
   ...
   * @param updatedData
   */
  saveChanges(updatedData: AddressType): void {
    if (this.registerDetail && this.id) {
      ...
      this.generalService
        .updateRegisterType<AddressType>(
          customEndpoint,
          Number(this.id),
          updatedData
        )
