...
@Component({
  selector: 'app-offices-detail',
  templateUrl: './offices-detail.component.html',
  styleUrls: ['./offices-detail.component.scss'],
})
export class OfficesDetailComponent implements OnInit {
  endpoints = getApiEndpoints();
  titol: string = '';

  id: string | null = null;
  registerDetail: Offices | null = null;
  originalData: Offices | null = null;
  ...
  ENDPOINT = `${this.endpoints.OFFICES}`;
  icono: string = 'offices';
  redirectRoute: string = 'offices'; //ruta retorn al llistat
  titolKey = 'MENU.OFFICES';
...
*/
  ...
  editFields: Array<EditInterface> = [
    ...
    {
      field: 'analiticalCode',
      type: 'number',
      captionKey: 'analiticalCode',
    },
    {
      field: 'name',
      type: 'string',
    ...
    {
      field: 'notifyVisitsGroup',
      type: 'boolean',
      captionKey: 'notifyVisitsGroup',
    },
    {
      field: 'fileId',
      type: 'numeric',
  ...
  toggleSave(): void {
    ...
    if (updatedDataString) {
      const updatedData: Offices = JSON.parse(updatedDataString);
  ...
  toggleInsert(): void {
    ...
    if (insertDataString) {
      const insertData: Offices = JSON.parse(insertDataString);
  ...
  loadRegisterDetail(): void {
    ...
    if (!isNaN(idNum)) {
      ...
      this.generalService
        .getRegisterTypeById<Offices>(dynamicEndpoint, idNum)
        .subscribe({
          next: (register: Offices | null) => {
  ...
  submitNewRecord(insertData: any, language: number): void {
    ...
    this.generalService
      .createRegisterType<Offices>(customEndpoint, dataToSend as Offices)
      .subscribe({
        next: () => {
          Swal.fire({
            icon: 'success',
   ...
   * @param updatedData
   */
  saveChanges(updatedData: Offices): void {
