...
export class CountriesDetailComponent implements OnInit {
  ...
  icono: string = 'countries';
  redirectRoute: string = 'countries'; //ruta retorn al llistat
  id: string | null = null;
  registerDetail: Countries | null = null;
  originalData: Countries | null = null;
  ...
  toggleSave(): void {
    ...
    if (updatedDataString) {
      const updatedData: Countries = JSON.parse(updatedDataString);
  ...
  toggleInsert(): void {
    ...
    if (insertDataString) {
      const insertData: Countries = JSON.parse(insertDataString);
  ...
  loadRegisterDetail(): void {
    ...
    if (!isNaN(idNum)) {
      ...
      this.generalService
        .getRegisterTypeById<Countries>(dynamicEndpoint, idNum)
        .subscribe({
          next: (register: Countries | null) => {
   ...
   * @param insertData
   */
  submitNewRecord(insertData: Countries): void {
    ...
    this.generalService
      .createRegisterType<Countries>(customEndpoint, insertData)
      .subscribe({
        next: () => {
          Swal.fire({
            icon: 'success',
   ...
   * @param updatedData
   */
  saveChanges(updatedData: Countries): void {
