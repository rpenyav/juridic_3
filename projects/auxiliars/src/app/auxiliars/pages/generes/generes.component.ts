...
import { Generes, PaginatedResponse } from '../../interfaces/personal/generes';
...
export class GeneresComponent implements OnInit {
  ...
  updateTableData(data: Generes[]): void {
  ...
  create(registerTypesData: Generes) {
    console.log('Enviando datos al servidor:', registerTypesData);
    this.generalService
      .createRegisterType<Generes>(this.ENDPOINT, registerTypesData)
      .subscribe({
        next: (response) => {
  ...
  delete(nametype: Generes) {
    ...
    showCustomAlert(this.i18nService, {
      ...
      if (result.isConfirmed) {
        this.generalService
          .deleteRegisterType<Generes>(this.ENDPOINT, nametype.id!)
          .subscribe({
            next: (response) => {
