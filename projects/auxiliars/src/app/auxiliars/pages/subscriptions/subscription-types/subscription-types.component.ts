...
import {
  SubscriptionType,
  PaginatedResponse,
} from '../../../interfaces/subscriptions/subscription-types';
...
export class SubscriptionTypeComponent implements OnInit {
  ...
  updateTableData(data: SubscriptionType[]): void {
  ...
  create(addressTypesData: SubscriptionType) {
    console.log('Enviando datos al servidor:', addressTypesData);
    this.generalService
      .createRegisterType<SubscriptionType>(this.ENDPOINT, addressTypesData)
      .subscribe({
        next: (response) => {
   ...
   * @param addressTypes
   */
  delete(addressTypes: SubscriptionType) {
    ...
    showCustomAlert(this.i18nService, {
      ...
      if (result.isConfirmed) {
        this.generalService
          .deleteRegisterType<SubscriptionType>(this.ENDPOINT, addressTypes.id!)
          .subscribe({
            next: (response) => {
