...
import { SubscriptionType } from '../../../interfaces/subscriptions/subscription-types';
import { EditInterface } from '../../../interfaces/editInterface';

@Component({
  selector: 'app-subscription-types-detail',
  templateUrl: './subscription-types-detail.component.html',
...
export class SubscriptionTypeDetailComponent implements OnInit {
  ...
  id: string | null = null;
  registerDetail: SubscriptionType | null = null;
  originalData: SubscriptionType | null = null;
  ...
  redirectRoute: string = 'subscription-types'; //ruta retorn al llistat
  ...
  toggleSave(): void {
    ...
    if (updatedDataString) {
      const updatedData: SubscriptionType = JSON.parse(updatedDataString);
  ...
  toggleInsert(): void {
    ...
    if (insertDataString) {
      const insertData: SubscriptionType = JSON.parse(insertDataString);
  ...
  loadRegisterDetail(): void {
    ...
    if (!isNaN(idNum)) {
      ...
      this.generalService
        .getRegisterTypeById<SubscriptionType>(dynamicEndpoint, idNum)
        .subscribe({
          next: (register: SubscriptionType | null) => {
  ...
  submitNewRecord(insertData: any, language: number): void {
    ...
    this.generalService
      .createRegisterType<SubscriptionType>(
        customEndpoint,
        dataToSend as SubscriptionType
      )
      .subscribe({
        next: () => {
          Swal.fire({
   ...
   * @param updatedData
   */
  saveChanges(updatedData: SubscriptionType): void {
    if (this.registerDetail && this.id) {
      ...
      this.generalService
        .updateRegisterType<SubscriptionType>(
          customEndpoint,
          Number(this.id),
          updatedData
        )
