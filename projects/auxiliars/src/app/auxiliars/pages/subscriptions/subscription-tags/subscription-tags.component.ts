...
import {
  SubscriptionTag,
  PaginatedResponse,
} from '../../../interfaces/subscription-tags';

@Component({
  selector: 'app-subscription-tags',
  templateUrl: './subscription-tags.component.html',
  styleUrls: ['./subscription-tags.component.scss'],
})
export class SubscriptionTagsComponent implements OnInit {
  assetsBaseUrl = '/assets/';
  endpoints = getApiEndpoints();
  ENDPOINT = `${this.endpoints.SUBSCRIPTION_TAGS}`;
  addressTypesData: SubscriptionTag[] = [];
  filteredRegistersData: SubscriptionTag[] = [];
  ...
  detailUrl: string = 'subscription-tag';
  ...
  updateTableData(data: SubscriptionTag[]): void {
  ...
  create(addressTypesData: SubscriptionTag) {
    console.log('Enviando datos al servidor:', addressTypesData);
    this.generalService
      .createRegisterType<SubscriptionTag>(this.ENDPOINT, addressTypesData)
      .subscribe({
        next: (response) => {
   ...
   * @param addressTypes
   */
  delete(addressTypes: SubscriptionTag) {
    ...
    showCustomAlert(this.i18nService, {
      ...
      if (result.isConfirmed) {
        this.generalService
          .deleteRegisterType<SubscriptionTag>(this.ENDPOINT, addressTypes.id!)
          .subscribe({
            next: (response) => {
