...
import { getApiEndpoints } from 'projects/auxiliars/src/app/constants/api-endpoints.constants';
import { SubscriptionProcedures } from '../../../interfaces/subscriptions/subscription-prodedures';

@Component({
  selector: 'app-subscription-procedures-detail',
  templateUrl: './subscription-procedures-detail.component.html',
  styleUrls: ['./subscription-procedures.component.scss'],
})
export class SubscriptionProceduresDetailComponent implements OnInit {
  endpoints = getApiEndpoints();
  titol: string = '';

  id: string | null = null;
  registerDetail: SubscriptionProcedures | null = null;
  originalData: SubscriptionProcedures | null = null;
  ...
  ENDPOINT = `${this.endpoints.SUBSCRIPTION_PROCEDURES}`;
  icono: string = 'subscription-procedures';
  redirectRoute: string = 'subscription-procedures'; //ruta retorn al llistat
  titolKey = 'MENU.SUBSCRIPTION_PROCEDURES';
  ...
  toggleSave(): void {
    ...
    if (updatedDataString) {
      const updatedData: SubscriptionProcedures = JSON.parse(updatedDataString);
  ...
  toggleInsert(): void {
    ...
    if (insertDataString) {
      const insertData: SubscriptionProcedures = JSON.parse(insertDataString);
  ...
  loadRegisterDetail(): void {
    ...
    if (!isNaN(idNum)) {
      ...
      this.generalService
        .getRegisterTypeById<SubscriptionProcedures>(dynamicEndpoint, idNum)
        .subscribe({
          next: (register: SubscriptionProcedures | null) => {
  ...
  submitNewRecord(insertData: any, language: number): void {
    ...
    this.generalService
      .createRegisterType<SubscriptionProcedures>(
        customEndpoint,
        dataToSend as SubscriptionProcedures
      )
      .subscribe({
        next: () => {
          Swal.fire({
   ...
   * @param updatedData
   */
  saveChanges(updatedData: SubscriptionProcedures): void {
    if (this.registerDetail && this.id) {
      ...
      this.generalService
        .updateRegisterType<SubscriptionProcedures>(
          customEndpoint,
          Number(this.id),
          updatedData
        )
