...
} from '../../../interfaces/subscriptions/subscription-coverages';
import { GeneralService } from '../../../services/general.service';
import { environment } from 'projects/auxiliars/src/environments/environment';

@Component({
  selector: 'app-subscription-coverages',
  templateUrl: './subscription-coverages.component.html',
  styleUrls: ['./subscription-coverages.component.scss'],
})
export class SubscriptionCoveragesComponent implements OnInit {
  ...
  detailUrl: string = 'subscription-coverages';
  ...
  addressTypesData: SubscriptionCoverage[] = [];
  filteredRegistersData: SubscriptionCoverage[] = [];
  ...
  updateTableData(data: SubscriptionCoverage[]): void {
  ...
  create(addressTypesData: SubscriptionCoverage) {
    console.log('Enviando datos al servidor:', addressTypesData);
    this.generalService
      .createRegisterType<SubscriptionCoverage>(this.ENDPOINT, addressTypesData)
      .subscribe({
        next: (response) => {
   ...
   * @param addressTypes
   */
  delete(addressTypes: SubscriptionCoverage) {
    ...
    showCustomAlert(this.i18nService, {
      ...
      if (result.isConfirmed) {
        this.generalService
          .deleteRegisterType<SubscriptionCoverage>(
            this.ENDPOINT,
            addressTypes.id!
          )
          .subscribe({
