...
import { SubscriptionStatus } from '../../../interfaces/subscriptions/subscription-status';

@Component({
  selector: 'app-subscription-status-detail',
  templateUrl: './subscription-status-detail.component.html',
  styleUrls: ['./subscription-status.component.scss'],
})
export class SubscriptionStatusDetailComponent implements OnInit {
  endpoints = getApiEndpoints();
  titol: string = '';

  id: string | null = null;
  registerDetail: SubscriptionStatus | null = null;
  originalData: SubscriptionStatus | null = null;
  ...
  ENDPOINT = `${this.endpoints.SUBSCRIPTION_STATUS}`;
  icono: string = 'subscription-status';
  redirectRoute: string = 'subscription-status'; //ruta retorn al llistat
  ...
  toggleSave(): void {
    ...
    if (updatedDataString) {
      const updatedData: SubscriptionStatus = JSON.parse(updatedDataString);
  ...
  toggleInsert(): void {
    ...
    if (insertDataString) {
      const insertData: SubscriptionStatus = JSON.parse(insertDataString);
  ...
  loadRegisterDetail(): void {
    ...
    if (!isNaN(idNum)) {
      ...
      this.generalService
        .getRegisterTypeById<SubscriptionStatus>(dynamicEndpoint, idNum)
        .subscribe({
          next: (register: SubscriptionStatus | null) => {
  ...
  submitNewRecord(insertData: any, language: number): void {
    ...
    this.generalService
      .createRegisterType<SubscriptionStatus>(
        customEndpoint,
        dataToSend as SubscriptionStatus
      )
      .subscribe({
        next: () => {
          Swal.fire({
   ...
   * @param updatedData
   */
  saveChanges(updatedData: SubscriptionStatus): void {
    if (this.registerDetail && this.id) {
      ...
      this.generalService
        .updateRegisterType<SubscriptionStatus>(
          customEndpoint,
          Number(this.id),
          updatedData
        )
