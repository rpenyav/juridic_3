...
import {
  SubscriptionStatus,
  PaginatedResponse,
} from '../../../interfaces/subscriptions/subscription-status';

@Component({
  selector: 'app-durations-status',
...
export class SubscriptionStatusComponent implements OnInit {
  ...
  detailUrl: string = 'subscription-status';
  ...
  addressTypesData: SubscriptionStatus[] = [];
  filteredRegistersData: SubscriptionStatus[] = [];
  ...
  updateTableData(data: SubscriptionStatus[]): void {
  ...
  create(addressTypesData: SubscriptionStatus) {
    console.log('Enviando datos al servidor:', addressTypesData);
    this.generalService
      .createRegisterType<SubscriptionStatus>(this.ENDPOINT, addressTypesData)
      .subscribe({
        next: (response) => {
   ...
   * @param addressTypes
   */
  delete(addressTypes: SubscriptionStatus) {
    ...
    showCustomAlert(this.i18nService, {
      ...
      if (result.isConfirmed) {
        this.generalService
          .deleteRegisterType<SubscriptionStatus>(
            this.ENDPOINT,
            addressTypes.id!
          )
          .subscribe({
