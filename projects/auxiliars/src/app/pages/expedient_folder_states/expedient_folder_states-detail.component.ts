...
import { ExpFolderStates } from 'src/app/interfaces/expedients/folder-states';
...
@Component({
  ...
  templateUrl: './expedient_folder_states-detail.component.html',
...
export class ExpedientFolderStatesDetailComponent implements OnInit {
  ...
  redirectRoute: string = 'folder-states'; //ruta retorn al llistat
  id: string | null = null;
  registerDetail: ExpFolderStates | null = null;
  originalData: ExpFolderStates | null = null;
  ...
  toggleSave(): void {
    ...
    if (updatedDataString) {
      const updatedData: ExpFolderStates = JSON.parse(updatedDataString);
  ...
  toggleInsert(): void {
    ...
    if (insertDataString) {
      const insertData: ExpFolderStates = JSON.parse(insertDataString);
  ...
  loadRegisterDetail(): void {
    ...
    if (!isNaN(idNum)) {
      ...
      this.generalService
        .getRegisterTypeById<ExpFolderStates>(dynamicEndpoint, idNum)
        .subscribe({
          next: (register: ExpFolderStates | null) => {
   ...
   * @param insertData
   */
  submitNewRecord(insertData: ExpFolderStates): void {
    ...
    this.generalService
      .createRegisterType<ExpFolderStates>(customEndpoint, insertData)
      .subscribe({
        next: () => {
          Swal.fire({
            icon: 'success',
   ...
   * @param updatedData
   */
  saveChanges(updatedData: ExpFolderStates): void {
    if (this.registerDetail && this.id) {
      ...
      this.generalService
        .updateRegisterType<ExpFolderStates>(
          customEndpoint,
          Number(this.id),
          updatedData
        )
