...
import {
  PaginatedResponse,
  ExpFolderStates,
} from 'src/app/interfaces/expedients/folder-states';
...
@Component({
  selector: 'app-expedient-folder-states',
  templateUrl: './expedient_folder_states.component.html',
...
export class ExpedientFolderStatesComponent implements OnInit {
  ...
  filteredRegistersData: ExpFolderStates[] = [];
  ...
  detailUrl: string = 'folder-states';
  ...
  updateTableData(data: ExpFolderStates[]): void {
  ...
  create(registerTypesData: ExpFolderStates) {
    console.log('Enviando datos al servidor:', registerTypesData);
    this.generalService
      .createRegisterType<ExpFolderStates>(this.ENDPOINT, registerTypesData)
      .subscribe({
        next: (response) => {
   ...
   * @param registerTypes
   */
  delete(nametype: ExpFolderStates) {
    ...
    showCustomAlert(this.txt, {
      ...
      if (result.isConfirmed) {
        this.generalService
          .deleteRegisterType<ExpFolderStates>(this.ENDPOINT, nametype.id!)
          .subscribe({
            next: (response) => {
