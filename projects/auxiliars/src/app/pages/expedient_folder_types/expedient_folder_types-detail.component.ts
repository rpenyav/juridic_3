...
import { ExpFolderTypes } from 'src/app/interfaces/expedients/folder-types';

import { GeneralService } from 'src/app/services/general.service';
import Swal from 'sweetalert2';

@Component({
  selector: 'app-expedient-folder-types-detail',
  templateUrl: './expedient_folder_types-detail.component.html',
...
export class ExpedientFolderTypesDetailComponent implements OnInit {
  ...
  redirectRoute: string = 'exp-folder-types'; //ruta retorn al llistat
  id: string | null = null;
  registerDetail: ExpFolderTypes | null = null;
  originalData: ExpFolderTypes | null = null;
  ...
  toggleSave(): void {
    ...
    if (updatedDataString) {
      const updatedData: ExpFolderTypes = JSON.parse(updatedDataString);
  ...
  toggleInsert(): void {
    ...
    if (insertDataString) {
      const insertData: ExpFolderTypes = JSON.parse(insertDataString);
  ...
  loadRegisterDetail(): void {
    ...
    if (!isNaN(idNum)) {
      ...
      this.generalService
        .getRegisterTypeById<ExpFolderTypes>(dynamicEndpoint, idNum)
        .subscribe({
          next: (register: ExpFolderTypes | null) => {
   ...
   * @param insertData
   */
  submitNewRecord(insertData: ExpFolderTypes): void {
    ...
    this.generalService
      .createRegisterType<ExpFolderTypes>(customEndpoint, insertData)
      .subscribe({
        next: () => {
          Swal.fire({
            icon: 'success',
   ...
   * @param updatedData
   */
  saveChanges(updatedData: ExpFolderTypes): void {
    if (this.registerDetail && this.id) {
      ...
      this.generalService
        .updateRegisterType<ExpFolderTypes>(
          customEndpoint,
          Number(this.id),
          updatedData
        )
