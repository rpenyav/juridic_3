...
import { ExpFolderSubjects } from 'src/app/interfaces/expedients/folder-subjects';

import { GeneralService } from 'src/app/services/general.service';
import Swal from 'sweetalert2';

@Component({
  selector: 'app-expedient-folder-subjects-detail',
  templateUrl: './expedient_folder_subjects-detail.component.html',
  styleUrls: ['./expedient_folder_subjects-detail.component.scss'],
})
export class ExpedientFolderSubjectsDetailComponent implements OnInit {
  assetsBaseUrl = environment.assetsBaseUrl;
  endpoints = getApiEndpoints();
  ENDPOINT = `${this.endpoints.EXPEDIENTS_FOLDER_STATES_ENDPOINT}`;
  iconoS: string = 'expedient_folder_subjects';
  icono: string = 'folders-browse';
  redirectRoute: string = 'folder-subjects'; //ruta retorn al llistat
  id: string | null = null;
  registerDetail: ExpFolderSubjects | null = null;
  originalData: ExpFolderSubjects | null = null;
  ...
  toggleSave(): void {
    ...
    if (updatedDataString) {
      const updatedData: ExpFolderSubjects = JSON.parse(updatedDataString);
  ...
  toggleInsert(): void {
    ...
    if (insertDataString) {
      const insertData: ExpFolderSubjects = JSON.parse(insertDataString);
  ...
  loadRegisterDetail(): void {
    ...
    if (!isNaN(idNum)) {
      ...
      this.generalService
        .getRegisterTypeById<ExpFolderSubjects>(dynamicEndpoint, idNum)
        .subscribe({
          next: (register: ExpFolderSubjects | null) => {
   ...
   * @param insertData
   */
  submitNewRecord(insertData: ExpFolderSubjects): void {
    ...
    this.generalService
      .createRegisterType<ExpFolderSubjects>(customEndpoint, insertData)
      .subscribe({
        next: () => {
          Swal.fire({
            icon: 'success',
   ...
   * @param updatedData
   */
  saveChanges(updatedData: ExpFolderSubjects): void {
    if (this.registerDetail && this.id) {
      ...
      this.generalService
        .updateRegisterType<ExpFolderSubjects>(
          customEndpoint,
          Number(this.id),
          updatedData
        )
