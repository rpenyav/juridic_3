...
import {
  PaginatedResponse,
  ExpFolderSubjects,
} from 'src/app/interfaces/expedients/folder-subjects';
...
@Component({
  selector: 'app-expedient-folder-subjects',
  templateUrl: './expedient_folder_subjects.component.html',
...
export class ExpedientFolderSubjectsComponent implements OnInit {
  ...
  filteredRegistersData: ExpFolderSubjects[] = [];
  ...
  detailUrl: string = 'folder-subjects';
  ...
  updateTableData(data: ExpFolderSubjects[]): void {
  ...
  create(registerTypesData: ExpFolderSubjects) {
    console.log('Enviando datos al servidor:', registerTypesData);
    this.generalService
      .createRegisterType<ExpFolderSubjects>(this.ENDPOINT, registerTypesData)
      .subscribe({
        next: (response) => {
   ...
   * @param registerTypes
   */
  delete(nametype: ExpFolderSubjects) {
    ...
    showCustomAlert(this.txt, {
      ...
      if (result.isConfirmed) {
        this.generalService
          .deleteRegisterType<ExpFolderSubjects>(this.ENDPOINT, nametype.id!)
          .subscribe({
            next: (response) => {
