...
import { FestivityDays } from 'src/app/interfaces/festivity/festivity-days';

import { GeneralService } from 'src/app/services/general.service';
import Swal from 'sweetalert2';

@Component({
  selector: 'app-festivity-days-detail',
  templateUrl: './festivity_days-detail.component.html',
...
export class FestivityDaysDetailComponent implements OnInit {
  ...
  icono: string = 'party';
  redirectRoute: string = 'festivity-days'; //ruta retorn al llistat
  id: string | null = null;
  registerDetail: FestivityDays | null = null;
  originalData: FestivityDays | null = null;
  ...
  toggleSave(): void {
    ...
    if (updatedDataString) {
      const updatedData: FestivityDays = JSON.parse(updatedDataString);
  ...
  toggleInsert(): void {
    ...
    if (insertDataString) {
      const insertData: FestivityDays = JSON.parse(insertDataString);
  ...
  loadRegisterDetail(): void {
    ...
    if (!isNaN(idNum)) {
      ...
      this.generalService
        .getRegisterTypeById<FestivityDays>(dynamicEndpoint, idNum)
        .subscribe({
          next: (register: FestivityDays | null) => {
   ...
   * @param insertData
   */
  submitNewRecord(insertData: FestivityDays): void {
    ...
    this.generalService
      .createRegisterType<FestivityDays>(customEndpoint, insertData)
      .subscribe({
        next: () => {
          Swal.fire({
            icon: 'success',
   ...
   * @param updatedData
   */
  saveChanges(updatedData: FestivityDays): void {
    if (this.registerDetail && this.id) {
      ...
      this.generalService
        .updateRegisterType<FestivityDays>(
          customEndpoint,
          Number(this.id),
          updatedData
        )
