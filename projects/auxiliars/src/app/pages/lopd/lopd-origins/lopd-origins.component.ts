...
import { LopdOrigins, PaginatedResponse } from 'src/app/interfaces/lopd/lopd-origins';

@Component({
  selector: 'app-lopd-origins',
  templateUrl: './lopd-origins.component.html',
  styleUrls: ['./lopd-origins.component.scss'],
})
export class LopdOriginsComponent implements OnInit {
  ...
  updateTableData(data: LopdOrigins[]): void {
  ...
  create(addressTypesData: LopdOrigins) {
    console.log('Enviando datos al servidor:', addressTypesData);
    this.generalService
      .createRegisterType<LopdOrigins>(this.ENDPOINT, addressTypesData)
      .subscribe({
        next: (response) => {
   ...
   * @param addressTypes
   */
  delete(addressTypes: LopdOrigins) {
    ...
    showCustomAlert(this.txt, {
      ...
      if (result.isConfirmed) {
        this.generalService
          .deleteRegisterType<LopdOrigins>(this.ENDPOINT, addressTypes.id!)
          .subscribe({
            next: (response) => {
