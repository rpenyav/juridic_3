...
import { LopdRegulations } from 'src/app/interfaces/lopd/lopd-regulations';

import { GeneralService } from 'src/app/services/general.service';
import Swal from 'sweetalert2';

@Component({
  selector: 'app-lopd-regulations-detail',
  templateUrl: './lopd-regulations-detail.component.html',
  styleUrls: ['./lopd-regulations-detail.component.scss'],
})
export class LopdRegulationsDetailComponent implements OnInit {
  endpoints = getApiEndpoints();
  titol: string = ""
 
  id: string | null = null;
  registerDetail: LopdRegulations | null = null;
  originalData: LopdRegulations | null = null;
  ...
  ENDPOINT = `${this.endpoints.LOPD_REGULATIONS}`;
  icono: string = 'lopd-regulations';
  redirectRoute: string = 'lopd-regulations'; //ruta retorn al llistat
  titolKey="SUBMENU.REGLOPD";
...
*/
  ...
  toggleSave(): void {
    ...
    if (updatedDataString) {
      const updatedData: LopdRegulations = JSON.parse(updatedDataString);
  ...
  toggleInsert(): void {
    ...
    if (insertDataString) {
      const insertData: LopdRegulations = JSON.parse(insertDataString);
  ...
  loadRegisterDetail(): void {
    ...
    if (!isNaN(idNum)) {
      ...
      this.generalService
        .getRegisterTypeById<LopdRegulations>(dynamicEndpoint, idNum)
        .subscribe({
          next: (register: LopdRegulations | null) => {
  ...
  submitNewRecord(insertData: any, language: number): void {
    ...
    this.generalService
      .createRegisterType<LopdRegulations>(customEndpoint, dataToSend as LopdRegulations)
      .subscribe({
        next: () => {
          Swal.fire({
            icon: 'success',
   ...
   * @param updatedData
   */
  saveChanges(updatedData: LopdRegulations): void {
    if (this.registerDetail && this.id) {
      ...
      this.generalService
        .updateRegisterType<LopdRegulations>(
          customEndpoint,
          Number(this.id),
          updatedData
        )
