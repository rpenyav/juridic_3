...
import {
  PaginatedResponse,
  Purpose,
} from 'src/app/interfaces/communications/purpose';
...
@Component({
  selector: 'app-communication-purpose',
  templateUrl: './communication-purpose.component.html',
  styleUrls: ['./communication-purpose.component.scss'],
})
export class CommunicationPurposeComponent implements OnInit {
  assetsBaseUrl = environment.assetsBaseUrl;
  endpoints = getApiEndpoints();
  ENDPOINT = `${this.endpoints.COMMUNICATIONS_PURPOSE}`;
  addressTypesData: Purpose[] = [];
  filteredRegistersData: Purpose[] = [];
  ...
  updateTableData(data: Purpose[]): void {
  ...
  create(addressTypesData: Purpose) {
    console.log('Enviando datos al servidor:', addressTypesData);
    this.generalService
      .createRegisterType<Purpose>(this.ENDPOINT, addressTypesData)
      .subscribe({
        next: (response) => {
  ...
  delete(addressTypes: Purpose) {
    ...
    showCustomAlert(this.txt, {
      ...
      if (result.isConfirmed) {
        this.generalService
          .deleteRegisterType<Purpose>(this.ENDPOINT, addressTypes.id!)
          .subscribe({
            next: (response) => {
