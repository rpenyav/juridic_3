...
import {
  PaginatedResponse,
  FestivityYears,
} from 'src/app/interfaces/festivity/festivity-years';
...
@Component({
  selector: 'app-festivity-years',
  templateUrl: './festivity_years.component.html',
...
export class FestivityYearsComponent implements OnInit {
  ...
  filteredRegistersData: FestivityYears[] = [];
  currentSearchBody!: any[];
  pageNumber: number = 0;
  pageSize: number = 10;
  sortField: string = 'year'; // Camp per defecte per la ordenacio
  ...
  detailUrl: string = 'festivity-years';
  ...
  updateTableData(data: FestivityYears[]): void {
  ...
  create(registerTypesData: FestivityYears) {
    console.log('Enviando datos al servidor:', registerTypesData);
    this.generalService
      .createRegisterType<FestivityYears>(this.ENDPOINT, registerTypesData)
      .subscribe({
        next: (response) => {
   ...
   * @param registerTypes
   */
  delete(nametype: FestivityYears) {
    ...
    showCustomAlert(this.txt, {
      ...
      if (result.isConfirmed) {
        this.generalService
          .deleteRegisterType<FestivityYears>(this.ENDPOINT, nametype.id!)
          .subscribe({
            next: (response) => {
