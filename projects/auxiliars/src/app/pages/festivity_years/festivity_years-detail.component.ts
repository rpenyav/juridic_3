...
import { FestivityYears } from 'src/app/interfaces/festivity/festivity-years';

import { GeneralService } from 'src/app/services/general.service';
import Swal from 'sweetalert2';

@Component({
  selector: 'app-festivity-years-detail',
  templateUrl: './festivity_years-detail.component.html',
  styleUrls: ['./festivity_years-detail.component.scss'],
})
export class FestivityYearsDetailComponent implements OnInit {
  assetsBaseUrl = environment.assetsBaseUrl;
  endpoints = getApiEndpoints();
  ENDPOINT = `${this.endpoints.FESTIVITY_YEARS_ENDPOINT}`;
  iconoS: string = 'festivity_years';
  icono: string = 'party';
  redirectRoute: string = 'festivity-years'; //ruta retorn al llistat
  id: string | null = null;
  registerDetail: FestivityYears | null = null;
  originalData: FestivityYears | null = null;
  ...
  toggleSave(): void {
    ...
    if (updatedDataString) {
      const updatedData: FestivityYears = JSON.parse(updatedDataString);
  ...
  toggleInsert(): void {
    ...
    if (insertDataString) {
      const insertData: FestivityYears = JSON.parse(insertDataString);
  ...
  loadRegisterDetail(): void {
    ...
    if (!isNaN(idNum)) {
      ...
      this.generalService
        .getRegisterTypeById<FestivityYears>(dynamicEndpoint, idNum)
        .subscribe({
          next: (register: FestivityYears | null) => {
   ...
   * @param insertData
   */
  submitNewRecord(insertData: FestivityYears): void {
    ...
    this.generalService
      .createRegisterType<FestivityYears>(customEndpoint, insertData)
      .subscribe({
        next: () => {
          Swal.fire({
            icon: 'success',
   ...
   * @param updatedData
   */
  saveChanges(updatedData: FestivityYears): void {
    if (this.registerDetail && this.id) {
      ...
      this.generalService
        .updateRegisterType<FestivityYears>(
          customEndpoint,
          Number(this.id),
          updatedData
        )
