...
import { PurposeProfiles } from 'src/app/interfaces/communications/purpose-profiles';
import { GeneralService } from 'src/app/services/general.service';
import Swal from 'sweetalert2';

@Component({
  selector: 'app-communication-purpose-profiles-detail',
  templateUrl: './communication-purpose-profiles-detail.component.html',
  styleUrls: ['./communication-purpose-profiles-detail.component.scss'],
})
export class CommunicationPurposeProfilesDetailComponent implements OnInit {
  ...
  redirectRoute: string = `/${this.langUser}/communications/purpose-profiles`; //ruta retorn al llistat
  id: string | null = null;
  registerDetail: PurposeProfiles | null = null;
  originalData: PurposeProfiles | null = null;
  ...
  toggleSave(): void {
    ...
    if (updatedDataString) {
      const updatedData: PurposeProfiles = JSON.parse(updatedDataString);
  ...
  toggleInsert(): void {
    ...
    if (insertDataString) {
      const insertData: PurposeProfiles = JSON.parse(insertDataString);
  ...
  loadRegisterDetail(): void {
    ...
    if (!isNaN(idNum)) {
      ...
      this.generalService
        .getRegisterTypeById<PurposeProfiles>(dynamicEndpoint, idNum)
        .subscribe({
          next: (register: PurposeProfiles | null) => {
   ...
   * @param insertData
   */
  submitNewRecord(insertData: PurposeProfiles): void {
    ...
    this.generalService
      .createRegisterType<PurposeProfiles>(customEndpoint, insertData)
      .subscribe({
        next: () => {
          Swal.fire({
            icon: 'success',
   ...
   * @param updatedData
   */
  saveChanges(updatedData: PurposeProfiles): void {
    if (this.registerDetail && this.id) {
      ...
      this.generalService
        .updateRegisterType<PurposeProfiles>(
          customEndpoint,
          Number(this.id),
          updatedData
        )
