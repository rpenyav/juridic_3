...
import { SubscriptionParentals } from 'src/app/interfaces/subscriptions/subscription-parentals';

import { GeneralService } from 'src/app/services/general.service';
import Swal from 'sweetalert2';

@Component({
  selector: 'app-subscription-parentals-detail',
  templateUrl: './subscription-parentals-detail.component.html',
...
export class SubscriptionParentalsDetailComponent implements OnInit {
  ...
  id: string | null = null;
  registerDetail: SubscriptionParentals | null = null;
  originalData: SubscriptionParentals | null = null;
  ...
  ENDPOINT = `${this.endpoints.SUBSCRIPTION_PARENTALS}`;
  icono: string = 'subscription-parentals';
  redirectRoute: string = 'subscription-parentals'; //ruta retorn al llistat
  titolKey = 'MENU.SUBSCRIPTION_PARENTALS';
  ...
  toggleSave(): void {
    ...
    if (updatedDataString) {
      const updatedData: SubscriptionParentals = JSON.parse(updatedDataString);
  ...
  toggleInsert(): void {
    ...
    if (insertDataString) {
      const insertData: SubscriptionParentals = JSON.parse(insertDataString);
  ...
  loadRegisterDetail(): void {
    ...
    if (!isNaN(idNum)) {
      ...
      this.generalService
        .getRegisterTypeById<SubscriptionParentals>(dynamicEndpoint, idNum)
        .subscribe({
          next: (register: SubscriptionParentals | null) => {
  ...
  submitNewRecord(insertData: any, language: number): void {
    ...
    this.generalService
      .createRegisterType<SubscriptionParentals>(
        customEndpoint,
        dataToSend as SubscriptionParentals
      )
      .subscribe({
        next: () => {
          Swal.fire({
   ...
   * @param updatedData
   */
  saveChanges(updatedData: SubscriptionParentals): void {
    if (this.registerDetail && this.id) {
      ...
      this.generalService
        .updateRegisterType<SubscriptionParentals>(
          customEndpoint,
          Number(this.id),
          updatedData
        )
