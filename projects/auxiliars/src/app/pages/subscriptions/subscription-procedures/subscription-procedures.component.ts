...
import { SubscriptionProcedures, PaginatedResponse } from 'src/app/interfaces/subscriptions/subscription-prodedures';

@Component({
  selector: 'app-durations-procedures',
  templateUrl: './subscription-procedures.component.html',
  styleUrls: ['./subscription-procedures.component.scss'],
})
export class SubscriptionProceduresComponent implements OnInit {
  ...
  detailUrl: string = 'subscription-procedures';
  sortField: string = 'code'; // Camp per defecte per la ordenacio
  icono: string = 'subscription-procedures';
  ENDPOINT = `${this.endpoints.SUBSCRIPTION_PROCEDURES}`;
  addressTypesData: SubscriptionProcedures[] = [];
  filteredRegistersData: SubscriptionProcedures[] = [];
  ...
  updateTableData(data: SubscriptionProcedures[]): void {
  ...
  create(addressTypesData: SubscriptionProcedures) {
    console.log('Enviando datos al servidor:', addressTypesData);
    this.generalService
      .createRegisterType<SubscriptionProcedures>(this.ENDPOINT, addressTypesData)
      .subscribe({
        next: (response) => {
   ...
   * @param addressTypes
   */
  delete(addressTypes: SubscriptionProcedures) {
    ...
    showCustomAlert(this.txt, {
      ...
      if (result.isConfirmed) {
        this.generalService
          .deleteRegisterType<SubscriptionProcedures>(this.ENDPOINT, addressTypes.id!)
          .subscribe({
            next: (response) => {
