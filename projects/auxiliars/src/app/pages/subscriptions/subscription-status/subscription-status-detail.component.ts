...
import { SubscriptionStatus } from 'src/app/interfaces/subscriptions/subscription-status';
...
export class SubscriptionStatusDetailComponent implements OnInit {
  ...
  submitNewRecord(insertData: any, language: number): void {
    ...
    this.generalService
      .createRegisterType<SubscriptionStatus>(customEndpoint, dataToSend as SubscriptionStatus)
      .subscribe({
        next: () => {
          Swal.fire({
            icon: 'success',
