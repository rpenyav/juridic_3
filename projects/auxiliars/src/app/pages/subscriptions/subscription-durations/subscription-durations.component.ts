...
import {
  ...
  PaginatedResponse,
} from 'src/app/interfaces/subscriptions/subscription-duration';

@Component({
  selector: 'app-durations-coverages',
  templateUrl: './subscription-durations.component.html',
  styleUrls: ['./subscription-durations.component.scss'],
})
export class SubscriptionDurationsComponent implements OnInit {
  ...
  detailUrl: string = 'subscription-durations';
  ...
  addressTypesData: SubscriptionDuration[] = [];
  filteredRegistersData: SubscriptionDuration[] = [];
  ...
  updateTableData(data: SubscriptionDuration[]): void {
  ...
  create(addressTypesData: SubscriptionDuration) {
    console.log('Enviando datos al servidor:', addressTypesData);
    this.generalService
      .createRegisterType<SubscriptionDuration>(this.ENDPOINT, addressTypesData)
      .subscribe({
        next: (response) => {
   ...
   * @param addressTypes
   */
  delete(addressTypes: SubscriptionDuration) {
    ...
    showCustomAlert(this.txt, {
      ...
      if (result.isConfirmed) {
        this.generalService
          .deleteRegisterType<SubscriptionDuration>(
            this.ENDPOINT,
            addressTypes.id!
          )
          .subscribe({
