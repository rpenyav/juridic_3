...
import { RelationTypes, PaginatedResponse } from 'src/app/interfaces/personal/relation_types';
...
export class RelationTypesComponent implements OnInit {
  ...
  updateTableData(data: RelationTypes[]): void {
  ...
  create(addressTypesData: RelationTypes) {
    console.log('Enviando datos al servidor:', addressTypesData);
    this.generalService
      .createRegisterType<RelationTypes>(this.ENDPOINT, addressTypesData)
      .subscribe({
        next: (response) => {
   ...
   * @param addressTypes
   */
  delete(addressTypes: RelationTypes) {
    ...
    showCustomAlert(this.txt, {
      ...
      if (result.isConfirmed) {
        this.generalService
          .deleteRegisterType<RelationTypes>(this.ENDPOINT, addressTypes.id!)
          .subscribe({
            next: (response) => {
