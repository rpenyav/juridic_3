...
import { RelationTypes } from 'src/app/interfaces/personal/relation_types';

import { GeneralService } from 'src/app/services/general.service';
import Swal from 'sweetalert2';

@Component({
  selector: 'app-relation-types-detail',
  templateUrl: './relation-types-detail.component.html',
  styleUrls: ['./relation-types.component.scss'],
})
export class RelationTypesDetailComponent implements OnInit {
  endpoints = getApiEndpoints();
  titol: string = ""
 
  id: string | null = null;
  registerDetail: RelationTypes | null = null;
  originalData: RelationTypes | null = null;
  ...
  ENDPOINT = `${this.endpoints.RELATION_TYPES}`;
  icono: string = 'relation-types';
  redirectRoute: string = 'relation-types'; //ruta retorn al llistat
  titolKey="SUBMENU.RELATION_TYPES";
  ...
  toggleSave(): void {
    ...
    if (updatedDataString) {
      const updatedData: RelationTypes = JSON.parse(updatedDataString);
  ...
  toggleInsert(): void {
    ...
    if (insertDataString) {
      const insertData: RelationTypes = JSON.parse(insertDataString);
  ...
  loadRegisterDetail(): void {
    ...
    if (!isNaN(idNum)) {
      ...
      this.generalService
        .getRegisterTypeById<RelationTypes>(dynamicEndpoint, idNum)
        .subscribe({
          next: (register: RelationTypes | null) => {
  ...
  submitNewRecord(insertData: any, language: number): void {
    ...
    this.generalService
      .createRegisterType<RelationTypes>(customEndpoint, dataToSend as RelationTypes)
      .subscribe({
        next: () => {
          Swal.fire({
            icon: 'success',
   ...
   * @param updatedData
   */
  saveChanges(updatedData: RelationTypes): void {
    if (this.registerDetail && this.id) {
      ...
      this.generalService
        .updateRegisterType<RelationTypes>(
          customEndpoint,
          Number(this.id),
          updatedData
        )
