...
import { Municipalities } from 'src/app/interfaces/localizations/municipalities';

import { GeneralService } from 'src/app/services/general.service';
import Swal from 'sweetalert2';

@Component({
  selector: 'app-municipalities-detail',
  templateUrl: './municipalities-detail.component.html',
  styleUrls: ['./municipalities-detail.component.scss'],
})
export class MunicipalitiesDetailComponent implements OnInit {
  endpoints = getApiEndpoints();
  titol: string = ""
 
  id: string | null = null;
  registerDetail: Municipalities | null = null;
  originalData: Municipalities | null = null;
  ...
  redirectRoute: string = 'municipalities'; //ruta retorn al llistat
...
*/
  ...
  editFields: Array<EditInterface>= [
    ...
    },
    {
      ...
      autocomplete:
      {
        url: this.endpoints.PROVINCIES_ENDPOINT,
        returnField: null,
        showFields:[
                    {name:"name"}
                   ]
    ...
    },
    {
      ...
      autocomplete:
      {
        url: this.endpoints.COMARCS_ENDPOINT,
        returnField: null,
        showFields:[
                    {name:"name"}
                   ]
  ...
  toggleSave(): void {
    ...
    if (updatedDataString) {
      const updatedData: Municipalities = JSON.parse(updatedDataString);
  ...
  toggleInsert(): void {
    ...
    if (insertDataString) {
      const insertData: Municipalities = JSON.parse(insertDataString);
  ...
  loadRegisterDetail(): void {
    ...
    if (!isNaN(idNum)) {
      ...
      this.generalService
        .getRegisterTypeById<Municipalities>(dynamicEndpoint, idNum)
        .subscribe({
          next: (register: Municipalities | null) => {
  ...
  submitNewRecord(insertData: any, language: number): void {
    ...
    this.generalService
      .createRegisterType<Municipalities>(customEndpoint, dataToSend as Municipalities)
      .subscribe({
        next: () => {
          Swal.fire({
            icon: 'success',
   ...
   * @param updatedData
   */
  saveChanges(updatedData: Municipalities): void {
    if (this.registerDetail && this.id) {
      ...
      this.generalService
        .updateRegisterType<Municipalities>(
          customEndpoint,
          Number(this.id),
          updatedData
        )
