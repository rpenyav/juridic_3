...
import { LegalRepresentations } from 'src/app/interfaces/personal/legal-representations';

import { GeneralService } from 'src/app/services/general.service';
import Swal from 'sweetalert2';

@Component({
  selector: 'app-legal-representations-detail',
  templateUrl: './legal-representations-detail.component.html',
  styleUrls: ['./legal-representations-detail.component.scss'],
})
export class LegalRepresentationsDetailComponent implements OnInit {
  endpoints = getApiEndpoints();
  titol: string = ""
 
  id: string | null = null;
  registerDetail: LegalRepresentations | null = null;
  originalData: LegalRepresentations | null = null;
  ...
  redirectRoute: string = 'legal-representations'; //ruta retorn al llistat
...
*/
  ...
  toggleSave(): void {
    ...
    if (updatedDataString) {
      const updatedData: LegalRepresentations = JSON.parse(updatedDataString);
  ...
  toggleInsert(): void {
    ...
    if (insertDataString) {
      const insertData: LegalRepresentations = JSON.parse(insertDataString);
  ...
  loadRegisterDetail(): void {
    ...
    if (!isNaN(idNum)) {
      ...
      this.generalService
        .getRegisterTypeById<LegalRepresentations>(dynamicEndpoint, idNum)
        .subscribe({
          next: (register: LegalRepresentations | null) => {
  ...
  submitNewRecord(insertData: any, language: number): void {
    ...
    this.generalService
      .createRegisterType<LegalRepresentations>(customEndpoint, dataToSend as LegalRepresentations)
      .subscribe({
        next: () => {
          Swal.fire({
            icon: 'success',
   ...
   * @param updatedData
   */
  saveChanges(updatedData: LegalRepresentations): void {
    if (this.registerDetail && this.id) {
      ...
      this.generalService
        .updateRegisterType<LegalRepresentations>(
          customEndpoint,
          Number(this.id),
          updatedData
        )
