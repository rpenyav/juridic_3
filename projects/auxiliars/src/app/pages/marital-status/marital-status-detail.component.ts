...
import { MaritalStatus } from 'src/app/interfaces/personal/marital-status';

import { GeneralService } from 'src/app/services/general.service';
import Swal from 'sweetalert2';

@Component({
  selector: 'app-marital-status-detail',
  templateUrl: './marital-status-detail.component.html',
  styleUrls: ['./marital-status-detail.component.scss'],
})
export class MaritalStatusDetailComponent implements OnInit {
  endpoints = getApiEndpoints();
  titol: string = ""
 
  id: string | null = null;
  registerDetail: MaritalStatus | null = null;
  originalData: MaritalStatus | null = null;
  ...
  ENDPOINT = `${this.endpoints.MARITAL_STATUS}`;
  icono: string = 'marital-status';
  redirectRoute: string = 'marital-status'; //ruta retorn al llistat
  titolKey="MENU.ESTATCIVIL";
...
*/
  ...
  toggleSave(): void {
    ...
    if (updatedDataString) {
      const updatedData: MaritalStatus = JSON.parse(updatedDataString);
  ...
  toggleInsert(): void {
    ...
    if (insertDataString) {
      const insertData: MaritalStatus = JSON.parse(insertDataString);
  ...
  loadRegisterDetail(): void {
    ...
    if (!isNaN(idNum)) {
      ...
      this.generalService
        .getRegisterTypeById<MaritalStatus>(dynamicEndpoint, idNum)
        .subscribe({
          next: (register: MaritalStatus | null) => {
  ...
  submitNewRecord(insertData: any, language: number): void {
    ...
    this.generalService
      .createRegisterType<MaritalStatus>(customEndpoint, dataToSend as MaritalStatus)
      .subscribe({
        next: () => {
          Swal.fire({
            icon: 'success',
   ...
   * @param updatedData
   */
  saveChanges(updatedData: MaritalStatus): void {
    if (this.registerDetail && this.id) {
      ...
      this.generalService
        .updateRegisterType<MaritalStatus>(
          customEndpoint,
          Number(this.id),
          updatedData
        )
