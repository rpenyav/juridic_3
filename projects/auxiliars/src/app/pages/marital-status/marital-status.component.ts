...
import { MaritalStatus, PaginatedResponse } from 'src/app/interfaces/personal/marital-status';
...
export class MaritalStatusComponent implements OnInit {
  ...
  updateTableData(data: MaritalStatus[]): void {
  ...
  create(addressTypesData: MaritalStatus) {
    console.log('Enviando datos al servidor:', addressTypesData);
    this.generalService
      .createRegisterType<MaritalStatus>(this.ENDPOINT, addressTypesData)
      .subscribe({
        next: (response) => {
   ...
   * @param addressTypes
   */
  delete(addressTypes: MaritalStatus) {
    ...
    showCustomAlert(this.txt, {
      ...
      if (result.isConfirmed) {
        this.generalService
          .deleteRegisterType<MaritalStatus>(this.ENDPOINT, addressTypes.id!)
          .subscribe({
            next: (response) => {
