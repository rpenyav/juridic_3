...
import { CompanyRelationTypes } from 'src/app/interfaces/company_relation_types';
import { GeneralService } from 'src/app/services/general.service';
import Swal from 'sweetalert2';

@Component({
  selector: 'app-company-relation-types-detail',
  templateUrl: './company_relation_types-detail.component.html',
  styleUrls: ['./company_relation_types-detail.component.scss'],
})
export class CompanyRelationTypesDetailComponent implements OnInit {
  assetsBaseUrl = environment.assetsBaseUrl;
  endpoints = getApiEndpoints();
  ENDPOINT = `${this.endpoints.COMP_REL_TYPES}`;
  icono: string = 'relEmpresesTypes';
  redirectRoute: string = 'company_relation_types'; //ruta retorn al llistat
  id: string | null = null;
  registerDetail: CompanyRelationTypes | null = null;
  originalData: CompanyRelationTypes | null = null;
  ...
  toggleSave(): void {
    ...
    if (updatedDataString) {
      const updatedData: CompanyRelationTypes = JSON.parse(updatedDataString);
  ...
  toggleInsert(): void {
    ...
    if (insertDataString) {
      const insertData: CompanyRelationTypes = JSON.parse(insertDataString);
  ...
  loadRegisterDetail(): void {
    ...
    if (!isNaN(idNum)) {
      ...
      this.generalService
        .getRegisterTypeById<CompanyRelationTypes>(dynamicEndpoint, idNum)
        .subscribe({
          next: (register: CompanyRelationTypes | null) => {
   ...
   * @param insertData
   */
  submitNewRecord(insertData: CompanyRelationTypes): void {
    ...
    this.generalService
      .createRegisterType<CompanyRelationTypes>(customEndpoint, insertData)
      .subscribe({
        next: () => {
          Swal.fire({
            icon: 'success',
   ...
   * @param updatedData
   */
  saveChanges(updatedData: CompanyRelationTypes): void {
    if (this.registerDetail && this.id) {
      ...
      this.generalService
        .updateRegisterType<CompanyRelationTypes>(
          customEndpoint,
          Number(this.id),
          updatedData
        )
