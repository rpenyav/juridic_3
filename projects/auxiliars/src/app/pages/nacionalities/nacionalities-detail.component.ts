...
import { Nacionalities } from 'src/app/interfaces/personal/nacionalities';

import { GeneralService } from 'src/app/services/general.service';
import Swal from 'sweetalert2';

@Component({
  selector: 'app-nacionalities-detail',
  templateUrl: './nacionalities-detail.component.html',
  styleUrls: ['./nacionalities-detail.component.scss'],
})
export class NacionalitiesDetailComponent implements OnInit {
  endpoints = getApiEndpoints();
  titol: string = ""
 
  id: string | null = null;
  registerDetail: Nacionalities | null = null;
  originalData: Nacionalities | null = null;
  ...
  ENDPOINT = `${this.endpoints.NACIONALITIES}`;
  icono: string = 'nacionalities';
  redirectRoute: string = 'nacionalities'; //ruta retorn al llistat
  titolKey="MENU.NACIONALITIES";
...
*/
  ...
  toggleSave(): void {
    ...
    if (updatedDataString) {
      const updatedData: Nacionalities = JSON.parse(updatedDataString);
  ...
  toggleInsert(): void {
    ...
    if (insertDataString) {
      const insertData: Nacionalities = JSON.parse(insertDataString);
  ...
  loadRegisterDetail(): void {
    ...
    if (!isNaN(idNum)) {
      ...
      this.generalService
        .getRegisterTypeById<Nacionalities>(dynamicEndpoint, idNum)
        .subscribe({
          next: (register: Nacionalities | null) => {
  ...
  submitNewRecord(insertData: any, language: number): void {
    ...
    this.generalService
      .createRegisterType<Nacionalities>(customEndpoint, dataToSend as Nacionalities)
      .subscribe({
        next: () => {
          Swal.fire({
            icon: 'success',
   ...
   * @param updatedData
   */
  saveChanges(updatedData: Nacionalities): void {
    if (this.registerDetail && this.id) {
      ...
      this.generalService
        .updateRegisterType<Nacionalities>(
          customEndpoint,
          Number(this.id),
          updatedData
        )
