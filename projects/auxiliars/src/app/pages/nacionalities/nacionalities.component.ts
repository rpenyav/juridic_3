...
import { Nacionalities, PaginatedResponse } from 'src/app/interfaces/personal/nacionalities';

@Component({
  selector: 'app-nacionalities',
  templateUrl: './nacionalities.component.html',
  styleUrls: ['./nacionalities.component.scss'],
})
export class NacionalitiesComponent implements OnInit {
  ...
  updateTableData(data: Nacionalities[]): void {
  ...
  create(addressTypesData: Nacionalities) {
    console.log('Enviando datos al servidor:', addressTypesData);
    this.generalService
      .createRegisterType<Nacionalities>(this.ENDPOINT, addressTypesData)
      .subscribe({
        next: (response) => {
   ...
   * @param addressTypes
   */
  delete(addressTypes: Nacionalities) {
    ...
    showCustomAlert(this.txt, {
      ...
      if (result.isConfirmed) {
        this.generalService
          .deleteRegisterType<Nacionalities>(this.ENDPOINT, addressTypes.id!)
          .subscribe({
            next: (response) => {
