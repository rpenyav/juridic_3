...
import { Cnae } from 'src/app/interfaces/cnae';
...
export class CnaeDetailComponent implements OnInit {
   ...
   * @param insertData
   */
  submitNewRecord(insertData: Cnae): void {
    ...
    this.generalService
      .createRegisterType<Cnae>(customEndpoint, insertData)
      .subscribe({
        next: () => {
          Swal.fire({
            icon: 'success',
