...
@Component({
  ...
  templateUrl: './pay-methods-detail.component.html',
...
export class PayMethodsDetailComponent implements OnInit {
  ...
  id: string | null = null;
  registerDetail: PayMethods | null = null;
  originalData: PayMethods | null = null;
  ...
  toggleSave(): void {
    ...
    if (updatedDataString) {
      const updatedData: PayMethods = JSON.parse(updatedDataString);
  ...
  toggleInsert(): void {
    ...
    if (insertDataString) {
      const insertData: PayMethods = JSON.parse(insertDataString);
  ...
  loadRegisterDetail(): void {
    ...
    if (!isNaN(idNum)) {
      ...
      this.generalService
        .getRegisterTypeById<PayMethods>(dynamicEndpoint, idNum)
        .subscribe({
          next: (register: PayMethods | null) => {
   ...
   * @param insertData
   */
  submitNewRecord(insertData: PayMethods): void {
    ...
    this.generalService
      .createRegisterType<PayMethods>(customEndpoint, insertData)
      .subscribe({
        next: () => {
          Swal.fire({
            icon: 'success',
   ...
   * @param updatedData
   */
  saveChanges(updatedData: PayMethods): void {
