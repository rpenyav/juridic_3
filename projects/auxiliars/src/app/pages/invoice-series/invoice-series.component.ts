...
import { InvoiceSeries, PaginatedResponse } from 'src/app/interfaces/invoice-series';

@Component({
  selector: 'app-invoice-series',
  templateUrl: './invoice-series.component.html',
  styleUrls: ['./invoice-series.component.scss'],
})
export class InvoiceSeriesComponent implements OnInit {
  ...
  updateTableData(data: InvoiceSeries[]): void {
  ...
  create(addressTypesData: InvoiceSeries) {
    console.log('Enviando datos al servidor:', addressTypesData);
    this.generalService
      .createRegisterType<InvoiceSeries>(this.ENDPOINT, addressTypesData)
      .subscribe({
        next: (response) => {
   ...
   * @param addressTypes
   */
  delete(addressTypes: InvoiceSeries) {
    ...
    showCustomAlert(this.txt, {
      ...
      if (result.isConfirmed) {
        this.generalService
          .deleteRegisterType<InvoiceSeries>(this.ENDPOINT, addressTypes.id!)
          .subscribe({
            next: (response) => {
