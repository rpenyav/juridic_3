...
import { PaginatedResponse, VisitTypes } from 'src/app/interfaces/visit-types';
...
export class VisitTypesComponent implements OnInit {
  ...
  addressTypesData: VisitTypes[] = [];
  filteredRegistersData: VisitTypes[] = [];
  ...
  updateTableData(data: VisitTypes[]): void {
  ...
  create(addressTypesData: VisitTypes) {
    console.log('Enviando datos al servidor:', addressTypesData);
    this.generalService
      .createRegisterType<VisitTypes>(this.ENDPOINT, addressTypesData)
      .subscribe({
        next: (response) => {
   ...
   * @param addressTypes
   */
  delete(addressTypes: VisitTypes) {
    ...
    showCustomAlert(this.txt, {
      ...
      if (result.isConfirmed) {
        this.generalService
          .deleteRegisterType<VisitTypes>(this.ENDPOINT, addressTypes.id!)
          .subscribe({
            next: (response) => {
