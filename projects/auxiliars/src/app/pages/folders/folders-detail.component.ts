...
import { Folders } from 'src/app/interfaces/folders';
...
export class FoldersDetailComponent implements OnInit {
  ...
  id: string | null = null;
  registerDetail: Folders | null = null;
  originalData: Folders | null = null;
  ...
  toggleSave(): void {
    ...
    if (updatedDataString) {
      const updatedData: Folders = JSON.parse(updatedDataString);
  ...
  toggleInsert(): void {
    ...
    if (insertDataString) {
      const insertData: Folders = JSON.parse(insertDataString);
  ...
  loadRegisterDetail(): void {
    ...
    if (!isNaN(idNum)) {
      ...
      this.generalService
        .getRegisterTypeById<Folders>(dynamicEndpoint, idNum)
        .subscribe({
          next: (register: Folders | null) => {
   ...
   * @param insertData
   */
  submitNewRecord(insertData: Folders): void {
    ...
    this.generalService
      .createRegisterType<Folders>(customEndpoint, insertData)
      .subscribe({
        next: () => {
          Swal.fire({
            icon: 'success',
   ...
   * @param updatedData
   */
  saveChanges(updatedData: Folders): void {
