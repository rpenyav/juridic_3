...
import { Professions } from 'src/app/interfaces/personal/professions';
...
@Component({
  ...
  styleUrls: ['./professions-detail.component.scss'],
})
export class ProfessionsDetailComponent implements OnInit {
  ...
  id: string | null = null;
  registerDetail: Professions | null = null;
  originalData: Professions | null = null;
  ...
  ENDPOINT = `${this.endpoints.PROFESSIONS}`;
  icono: string = 'professions';
  redirectRoute: string = 'professions'; //ruta retorn al llistat
  titolKey="SUBMENU.PROFESSIONS";
  ...
  toggleSave(): void {
    ...
    if (updatedDataString) {
      const updatedData: Professions = JSON.parse(updatedDataString);
  ...
  toggleInsert(): void {
    ...
    if (insertDataString) {
      const insertData: Professions = JSON.parse(insertDataString);
  ...
  loadRegisterDetail(): void {
    ...
    if (!isNaN(idNum)) {
      ...
      this.generalService
        .getRegisterTypeById<Professions>(dynamicEndpoint, idNum)
        .subscribe({
          next: (register: Professions | null) => {
  ...
  submitNewRecord(insertData: any, language: number): void {
    ...
    this.generalService
      .createRegisterType<Professions>(customEndpoint, dataToSend as Professions)
      .subscribe({
        next: () => {
          Swal.fire({
            icon: 'success',
   ...
   * @param updatedData
   */
  saveChanges(updatedData: Professions): void {
    if (this.registerDetail && this.id) {
      ...
      this.generalService
        .updateRegisterType<Professions>(
          customEndpoint,
          Number(this.id),
          updatedData
        )
