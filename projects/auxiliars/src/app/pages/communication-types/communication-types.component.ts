...
import {
  PaginatedResponse,
  ComTypes,
} from 'src/app/interfaces/communications/types';
...
export class CommunicationTypesComponent implements OnInit {
  ...
  filteredRegistersData: ComTypes[] = [];
    ...
    this.generalService
      ...
      .subscribe({
        next: (data: PaginatedResponse) => {
          this.addressTypesData = data.lista;
  ...
  onSorted(sortData: { key: string; direction: 'ASC' | 'DESC' }) {
    ...
    this.generalService
      ...
      .subscribe((response) => {
        this.updateTableData(response.lista);
      });
  }

  updateTableData(data: ComTypes[]): void {
  ...
  create(addressTypesData: ComTypes) {
    console.log('Enviando datos al servidor:', addressTypesData);
    this.generalService
      .createRegisterType<ComTypes>(this.ENDPOINT, addressTypesData)
      .subscribe({
        next: (response) => {
  ...
  delete(addressTypes: ComTypes) {
    ...
    showCustomAlert(this.txt, {
      ...
      if (result.isConfirmed) {
        this.generalService
          .deleteRegisterType<ComTypes>(this.ENDPOINT, addressTypes.id!)
          .subscribe({
            next: (response) => {
