...
import { PersonTypes, PaginatedResponse } from 'src/app/interfaces/personal/person-types';
...
export class PersonTypesComponent implements OnInit {
  ...
  updateTableData(data: PersonTypes[]): void {
  ...
  create(addressTypesData: PersonTypes) {
    console.log('Enviando datos al servidor:', addressTypesData);
    this.generalService
      .createRegisterType<PersonTypes>(this.ENDPOINT, addressTypesData)
      .subscribe({
        next: (response) => {
   ...
   * @param addressTypes
   */
  delete(addressTypes: PersonTypes) {
    ...
    showCustomAlert(this.txt, {
      ...
      if (result.isConfirmed) {
        this.generalService
          .deleteRegisterType<PersonTypes>(this.ENDPOINT, addressTypes.id!)
          .subscribe({
            next: (response) => {
