...
import { PersonTypes } from 'src/app/interfaces/personal/person-types';

import { GeneralService } from 'src/app/services/general.service';
import Swal from 'sweetalert2';

@Component({
  selector: 'app-person-types-detail',
  templateUrl: './person-types-detail.component.html',
  styleUrls: ['./person-types-detail.component.scss'],
})
export class PersonTypesDetailComponent implements OnInit {
  endpoints = getApiEndpoints();
  titol: string = ""
 
  id: string | null = null;
  registerDetail: PersonTypes | null = null;
  originalData: PersonTypes | null = null;
  ...
  ENDPOINT = `${this.endpoints.PERSON_TYPES}`;
  icono: string = 'person-types';
  redirectRoute: string = 'person-types'; //ruta retorn al llistat
  titolKey="SUBMENU.TIPUSPERSON";
  ...
  toggleSave(): void {
    ...
    if (updatedDataString) {
      const updatedData: PersonTypes = JSON.parse(updatedDataString);
  ...
  toggleInsert(): void {
    ...
    if (insertDataString) {
      const insertData: PersonTypes = JSON.parse(insertDataString);
  ...
  loadRegisterDetail(): void {
    ...
    if (!isNaN(idNum)) {
      ...
      this.generalService
        .getRegisterTypeById<PersonTypes>(dynamicEndpoint, idNum)
        .subscribe({
          next: (register: PersonTypes | null) => {
  ...
  submitNewRecord(insertData: any, language: number): void {
    ...
    this.generalService
      .createRegisterType<PersonTypes>(customEndpoint, dataToSend as PersonTypes)
      .subscribe({
        next: () => {
          Swal.fire({
            icon: 'success',
   ...
   * @param updatedData
   */
  saveChanges(updatedData: PersonTypes): void {
    if (this.registerDetail && this.id) {
      ...
      this.generalService
        .updateRegisterType<PersonTypes>(
          customEndpoint,
          Number(this.id),
          updatedData
        )
