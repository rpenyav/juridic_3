...
import { Profiles } from 'src/app/interfaces/personal/profiles';
...
export class ProfilesDetailComponent implements OnInit {
  ...
  id: string | null = null;
  registerDetail: Profiles | null = null;
  originalData: Profiles | null = null;
  ...
  redirectRoute: string = 'profiles'; //ruta retorn al llistat
  ...
  toggleSave(): void {
    ...
    if (updatedDataString) {
      const updatedData: Profiles = JSON.parse(updatedDataString);
  ...
  toggleInsert(): void {
    ...
    if (insertDataString) {
      const insertData: Profiles = JSON.parse(insertDataString);
  ...
  loadRegisterDetail(): void {
    ...
    if (!isNaN(idNum)) {
      ...
      this.generalService
        .getRegisterTypeById<Profiles>(dynamicEndpoint, idNum)
        .subscribe({
          next: (register: Profiles | null) => {
  ...
  submitNewRecord(insertData: any, language: number): void {
    ...
    this.generalService
      .createRegisterType<Profiles>(customEndpoint, dataToSend as Profiles)
      .subscribe({
        next: () => {
          Swal.fire({
            icon: 'success',
   ...
   * @param updatedData
   */
  saveChanges(updatedData: Profiles): void {
