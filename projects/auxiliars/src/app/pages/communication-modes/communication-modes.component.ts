...
import {
  PaginatedResponse,
  ComModes,
} from 'src/app/interfaces/communications/modes';
...
@Component({
  selector: 'app-communication-modes',
  templateUrl: './communication-modes.component.html',
  styleUrls: ['./communication-modes.component.scss'],
})
export class CommunicationModesComponent implements OnInit {
  assetsBaseUrl = environment.assetsBaseUrl;
  endpoints = getApiEndpoints();
  ENDPOINT = `${this.endpoints.COMMUNICATIONS_MODES}`;
  addressTypesData: ComModes[] = [];
  filteredRegistersData: ComModes[] = [];
  currentSearchBody!: any[];
  pageNumber: number = 0;
  pageSize: number = 10;
  sortField: string = 'literalNameText'; // Camp per defecte per la ordenacio
  ...
  updateTableData(data: ComModes[]): void {
  ...
  create(addressTypesData: ComModes) {
    console.log('Enviando datos al servidor:', addressTypesData);
    this.generalService
      .createRegisterType<ComModes>(this.ENDPOINT, addressTypesData)
      .subscribe({
        next: (response) => {
   ...
   * @param addressTypes
   */
  delete(addressTypes: ComModes) {
    const titleWithName = `${this.txt.instant('FORM.desea_eliminar')} "${
      addressTypes.literalNameText
    }"?`;
    ...
    showCustomAlert(this.txt, {
      ...
      if (result.isConfirmed) {
        this.generalService
          .deleteRegisterType<ComModes>(this.ENDPOINT, addressTypes.id!)
          .subscribe({
            next: (response) => {
