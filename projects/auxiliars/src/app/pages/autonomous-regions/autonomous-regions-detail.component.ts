...
import { Autonomies } from 'src/app/interfaces/autonomies';

import { GeneralService } from 'src/app/services/general.service';
import Swal from 'sweetalert2';

@Component({
  selector: 'app-autonomous-regions-detail',
  templateUrl: './autonomous-regions-detail.component.html',
...
export class AutonomousRegionsDetailComponent implements OnInit {
  ...
  icono: string = 'autonomies';
  redirectRoute: string = 'addresstype'; //ruta retorn al llistat
  id: string | null = null;
  registerDetail: Autonomies | null = null;
  originalData: Autonomies | null = null;
  ...
  toggleSave(): void {
    ...
    if (updatedDataString) {
      const updatedData: Autonomies = JSON.parse(updatedDataString);
  ...
  toggleInsert(): void {
    ...
    if (insertDataString) {
      const insertData: Autonomies = JSON.parse(insertDataString);
  ...
  loadRegisterDetail(): void {
    ...
    if (!isNaN(idNum)) {
      ...
      this.generalService
        .getRegisterTypeById<Autonomies>(dynamicEndpoint, idNum)
        .subscribe({
          next: (register: Autonomies | null) => {
   ...
   * @param insertData
   */
  submitNewRecord(insertData: Autonomies): void {
    ...
    this.generalService
      .createRegisterType<Autonomies>(customEndpoint, insertData)
      .subscribe({
        next: () => {
          Swal.fire({
            icon: 'success',
   ...
   * @param updatedData
   */
  saveChanges(updatedData: Autonomies): void {
