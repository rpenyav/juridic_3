...
import { PaginatedResponse, Autonomies } from 'src/app/interfaces/autonomies';
...
@Component({
  ...
  templateUrl: './autonomous-regions.component.html',
...
export class AutonomousRegionsComponent implements OnInit {
  ...
  filteredRegistersData: Autonomies[] = [];
  ...
  detailUrl: string = 'autonomous-regions';
  ...
  updateTableData(data: Autonomies[]): void {
  ...
  create(registerTypesData: Autonomies) {
    console.log('Enviando datos al servidor:', registerTypesData);
    this.generalService
      .createRegisterType<Autonomies>(this.ENDPOINT, registerTypesData)
      .subscribe({
        next: (response) => {
  ...
  delete(nametype: Autonomies) {
    ...
    showCustomAlert(this.txt, {
      ...
      if (result.isConfirmed) {
        this.generalService
          .deleteRegisterType<Autonomies>(this.ENDPOINT, nametype.id!)
          .subscribe({
            next: (response) => {
