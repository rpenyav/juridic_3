...
import { Offices, PaginatedResponse } from 'src/app/interfaces/offices';
...
export class OfficesComponent implements OnInit {
  ...
  columnsConfig: {
    ...
    {
      key: 'notifyVisitsGroup',
      label: 'notifyVisitsGroup',
      sortable: true,
      direction: 'ASC',
      width: '10%',
    },
  ...
  updateTableData(data: Offices[]): void {
  ...
  create(addressTypesData: Offices) {
    console.log('Enviando datos al servidor:', addressTypesData);
    this.generalService
      .createRegisterType<Offices>(this.ENDPOINT, addressTypesData)
      .subscribe({
        next: (response) => {
  ...
  delete(addressTypes: Offices) {
    ...
    showCustomAlert(this.txt, {
      ...
      if (result.isConfirmed) {
        this.generalService
          .deleteRegisterType<Offices>(this.ENDPOINT, addressTypes.id!)
          .subscribe({
            next: (response) => {
