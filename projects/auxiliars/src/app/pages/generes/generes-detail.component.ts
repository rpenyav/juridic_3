...
import { Generes } from 'src/app/interfaces/personal/generes';

import { GeneralService } from 'src/app/services/general.service';
import Swal from 'sweetalert2';

@Component({
  selector: 'app-generes-detail',
  templateUrl: './generes-detail.component.html',
...
export class GeneresDetailComponent implements OnInit {
  ...
  icono: string = 'generes';
  redirectRoute: string = 'genders'; //ruta retorn al llistat
  id: string | null = null;
  registerDetail: Generes | null = null;
  originalData: Generes | null = null;
  ...
  toggleSave(): void {
    ...
    if (updatedDataString) {
      const updatedData: Generes = JSON.parse(updatedDataString);
  ...
  toggleInsert(): void {
    ...
    if (insertDataString) {
      const insertData: Generes = JSON.parse(insertDataString);
  ...
  loadRegisterDetail(): void {
    ...
    if (!isNaN(idNum)) {
      ...
      this.generalService
        .getRegisterTypeById<Generes>(dynamicEndpoint, idNum)
        .subscribe({
          next: (register: Generes | null) => {
   ...
   * @param insertData
   */
  submitNewRecord(insertData: Generes): void {
    ...
    this.generalService
      .createRegisterType<Generes>(customEndpoint, insertData)
      .subscribe({
        next: () => {
          Swal.fire({
            icon: 'success',
   ...
   * @param updatedData
   */
  saveChanges(updatedData: Generes): void {
